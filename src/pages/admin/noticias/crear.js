...
export default function CrearNoticia() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.message || "Error creando noticia");
      }
    } catch (err) {
      setError("Error creando noticia");
    }
    setSaving(false);
  };
  ...
  return (
    <div style={{ maxWidth: 600, margin: '2rem auto', background: '#fff', borderRadius: 8, boxShadow: '0 2px 8px #0001', padding: 24 }}>
      <h1 style={{ marginBottom: 24 }}>Crear Nueva Noticia</h1>
      <form onSubmit={handleSubmit}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <button type="submit" disabled={saving} style={{ background: '#1976d2', color: '#fff', border: 'none', borderRadius: 4, padding: '10px 24px', fontSize: 16, marginTop: 8, cursor: 'pointer' }}>{saving ? "Guardando..." : "Crear Noticia"}</button>
        </div>
      </form>
    </div>
  );
